...
> **문서 버전:** 1.6.0  
> **최종 갱신일:** 2026-09-16  
> **프로젝트 성격:** 다문화가정 및 외국인 학습자를 위한 실용 한국어 회화, 어휘, 문법, 문화, 퀴즈, 게이미피케이션, 회원 인증, 음소 단위 발음 교정, 실시간 AI 프리토킹 롤플레이(Gemini Live), 홈 UI/UX 최적화, Supabase 클라우드 실시간 동기화 및 글로벌 리더보드 통합 학습 플랫폼
...
- **프론트엔드 스택:** React 19, TypeScript, Vite, Tailwind CSS, Lucide Icons, Canvas-Confetti
- **클라우드 백엔드 & 실시간 동기화 (Supabase PostgreSQL):**
  - 스마트폰, 태블릿, PC 어디서든 실시간 진도, 뱃지, 퀴즈 점수, 오답노트 100% 양방향 동기화
  - Supabase Realtime 채널 구독으로 멀티 디바이스 간 새로고침 없는 즉각 반영
  - 오프라인/게스트 모드와 클라우드 간의 무손실 자동 머지(Merge) 엔진 탑재
- **글로벌 리더보드 & 오답노트 (`/leaderboard`):**
  - **주간 학습 시간 랭킹**: 🥇🥈🥉 시상대(Podium) 카드, 다문화 글로벌 학습자 매칭, 학습자 본인 순위 강조 및 주간 초기화 타이머
  - **퀴즈 만점자 명예의 전당**: 45개 전 단원 100% 만점 달성자 기록 및 명예 뱃지
  - **나의 맞춤 오답노트**: 퀴즈에서 틀린 문제의 자동 기록, 오답/정답 비교, 문법 해설 및 원클릭 재시험 연동
- **홈 UI/UX & 정보 위계 최적화:**
...
- **회원 인증 & 프로필:** `AuthContext` + Supabase Auth 기반 회원가입, 로그인, 게스트 모드, 소셜 로그인(Google/Kakao), 9개국어 모국어(한국어, 영어, 베트남어, 중국어, 일본어, 러시아어, 스페인어, 몽골어, 태국어) 및 아바타 커스텀
- **음성/AI 기능:**
